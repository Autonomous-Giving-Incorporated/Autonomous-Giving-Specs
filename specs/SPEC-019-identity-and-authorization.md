...
| --- | --- | --- | --- | --- | --- |
| Dependencies | SPEC-002A, SPEC-006, SPEC-016, SPEC-028 | Related ADRs | ADR-004, ADR-006, ADR-013, ADR-014 | Related contracts | CONTRACT-003, CONTRACT-006, CONTRACT-008, CONTRACT-011 |
...
Downstream Fund Intel and Impact Relay calls that cross a process or repository boundary MUST carry an AGI-issued capability JWT whose claims validate as [CONTRACT-008](../contracts/CONTRACT-008-auth-context.md) ([SPEC-028](SPEC-028-agi-control-plane.md), [ADR-014](../adr/ADR-014-agi-control-plane.md)). The receiving capability MUST independently verify issuer, signature, audience, expiry, tenant, and capability, and MUST **deny by default** on unverified downstream context. An IdP session cookie on the caller is not a substitute for that verification.
...
11. Capability JWTs MUST use asymmetric signing and a published JWKS, explicit audience, tenant/project scope, capability scope, expiry, and `jti`. Tokens MUST NEVER appear in URLs. The browser MUST NEVER receive the service-role key or reusable cross-service secrets.
12. Dual approval is optional tenant policy ([CONTRACT-011](../contracts/CONTRACT-011-delegation-policy.md)); default remains single-authority. Human Approval still gates allocations ([ADR-006](../adr/ADR-006-human-approval.md)).
