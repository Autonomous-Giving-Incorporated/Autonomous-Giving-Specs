# Recovery runbook (preferred Render PostgreSQL path)

Informative operational placeholder for product teams. Customize RPO/RTO, contacts, and exact Render console steps in the product repository runbook. Platform expectations: [SPEC-025](../specs/SPEC-025-operations-deploy-and-scale.md), financial invariants: [SPEC-023](../specs/SPEC-023-financial-ledger-invariants.md).
...
1. Confirm Render PostgreSQL automatic backups are enabled for the environment tier.
...
3. **Restore Postgres** — use Render backup/PITR to a new database instance (prefer not overwriting until verified).
4. **Point staging/canary app** at restored DB; run integrity checks:
...
3. Process through the same verified, idempotent path as live webhooks.
